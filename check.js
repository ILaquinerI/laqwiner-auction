...
async function save(id){const el=document.getElementById('a'+id);if(!el)return;const raw=String(el.value??'').trim();if(raw===''){toast('Введи сумму');el.focus();return;}const amount=Math.max(0,Math.round(Number(raw)||0));try{let d=null,lastErr=null;for(let attempt=0;attempt<3;attempt++){try{d=await api('/api/tank',{method:'POST',body:JSON.stringify({id,amount})});if(d&&d.state)break;throw Error('Сервер не вернул сохранённое состояние')}catch(err){lastErr=err;if(attempt<2)await new Promise(r=>setTimeout(r,350*(attempt+1)))}}if(!d||!d.state)throw lastErr||Error('Не удалось сохранить сумму');S=d.state;render();toast('Сумма сохранена: '+money(amount))}catch(e){toast('Ошибка: '+e.message)}}
...
function shuffleWheelItems(items){
 const base=[...items];
 if(base.length<3)return base.sort(()=>Math.random()-.5);
 const price=t=>String(Number(t.amount)||0);
 for(let attempt=0;attempt<250;attempt++){
  const a=[...base];
  for(let i=a.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[a[i],a[j]]=[a[j],a[i]];}
  let ok=true;
  for(let i=0;i<a.length;i++){if(price(a[i])===price(a[(i+1)%a.length])){ok=false;break;}}
  if(ok)return a;
 }
 // Fallback: interleave price groups so equal-price tanks are spread as far apart as possible.
 const groups=[...new Map(base.map(t=>[price(t),[]])).values()];
 for(const g of groups)g.sort(()=>Math.random()-.5);
 groups.sort((a,b)=>b.length-a.length);
 const out=[];
 while(groups.some(g=>g.length)){
  const choices=groups.filter(g=>g.length && (!out.length || price(g[0])!==price(out[out.length-1])));
  const g=(choices.length?choices:groups.filter(g=>g.length)).sort((a,b)=>b.length-a.length)[0];
  out.push(g.shift());
 }
 if(out.length>2 && price(out[0])===price(out[out.length-1])){
  for(let i=1;i<out.length-1;i++){if(price(out[i])!==price(out[0]) && price(out[i])!==price(out[out.length-1])){[out[0],out[i]]=[out[i],out[0]];break;}}
 }
 return out;
}
...
function hideSpinOverlay(){
 const o=document.getElementById('showOverlay');if(o){o.classList.remove('show','final');o.style.removeProperty('display');}
 const resultBox=document.querySelector('.wheelControls .result');if(resultBox)resultBox.style.display='';
}
function keepSpinOverlay(){const o=document.getElementById('showOverlay');if(o){o.classList.add('show');o.style.removeProperty('display');}}
