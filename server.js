...
async function handle(req,res){
 ...
 try {
  ...
  if(p==='/api/eliminate'&&req.method==='POST'){
    ...
    await saveState(); broadcast(); const remaining=state.tanks.filter(x=>x.alive!==false&&x.marked3!==true&&Number(x.amount)>0); return json(res,200,{ok:true,state:publicState(),remaining:remaining.length,winner:remaining.length===1?{id:remaining[0].id,name:remaining[0].name,image:remaining[0].image||''}:null});
  }
