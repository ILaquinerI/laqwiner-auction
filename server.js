...
const initial = () => ({version:6,round:1,tanks:JSON.parse(JSON.stringify(tanks)),history:[],recentDonations:[],orders:[],lastEliminatedId:null,timer:defaultTimer(),updatedAt:Date.now()});
let state = initial();
let previousStates = [];
...
function normalizeTanks(list){ return (list||[]).map(t=>({...t,amount:Number.isFinite(Number(t.amount))?Math.max(0,Number(t.amount)):0,weight:Number.isFinite(Number(t.weight))?Math.max(0,Math.min(1000000,Math.round(Number(t.weight)))):1,alive:t.alive!==false})); }
async function loadState(){
  try { const rows=await sb('auction_state?id=eq.1&select=state'); if(rows?.[0]?.state?.tanks?.length) { state=rows[0].state; state.timer={...defaultTimer(),...(state.timer||{})}; state.tanks=normalizeTanks(state.tanks); state.version=6; state.orders=Array.isArray(state.orders)?state.orders:[]; } else await saveState(); }
...
async function handle(req,res){
 ...
 try {
  ...
  if(['/api/state','/api/timer','/api/spin','/api/undo','/api/new-round','/api/reset','/api/order-status'].includes(p) && !isAdmin(req)) return json(res,403,{error:'Требуется вход администратора'});
  if(p==='/api/state'&&req.method==='PUT'){const body=await readBody(req);if(!body?.tanks?.length)return json(res,400,{error:'Некорректное состояние'});previousStates.push(publicState());if(previousStates.length>20)previousStates.shift();state={...body,version:6,updatedAt:Date.now(),timer:{...defaultTimer(),...(body.timer||{})},tanks:normalizeTanks(body.tanks),orders:Array.isArray(body.orders)?body.orders:[]};await saveState();broadcast();return json(res,200,publicState())}
  if(p==='/api/order'&&req.method==='POST'){const body=await readBody(req);const tankId=Number(body.tankId);const tank=state.tanks.find(t=>t.id===tankId);if(!tank)return json(res,400,{error:'Танк не найден'});const nickname=String(body.nickname||'Гость').trim().slice(0,40)||'Гость';const note=String(body.note||'').trim().slice(0,120);state.orders=Array.isArray(state.orders)?state.orders:[];state.orders.unshift({id:crypto.randomBytes(8).toString('hex'),tankId,tankName:tank.name,nickname,note,status:'new',createdAt:Date.now()});if(state.orders.length>200)state.orders=state.orders.slice(0,200);await saveState();broadcast();return json(res,200,{ok:true,state:publicState()})}
  if(p==='/api/order-status'&&req.method==='POST'){const body=await readBody(req);const order=Array.isArray(state.orders)?state.orders.find(o=>o.id===body.id):null;if(!order)return json(res,404,{error:'Заказ не найден'});order.status=String(body.status||'done');await saveState();broadcast();return json(res,200,{state:publicState()})}
