...
async function loadState(){
  try { const rows=await sb('auction_state?id=eq.1&select=state'); if(rows?.[0]?.state?.tanks?.length) { state=rows[0].state; state.timer={...defaultTimer(),...(state.timer||{})}; state.tanks=normalizeTanks(state.tanks); state.version=5; } else await saveState(); }
...
async function handle(req,res){
 ...
 try {
  ...
  if(p==='/api/spin'&&req.method==='POST'){const {a:active}=activeWeighted();if(active.length<2)return json(res,400,{error:'Нужно минимум 2 танка с донатами'});const target=randomPick();previousStates.push(publicState());if(previousStates.length>20)previousStates.shift();target.alive=false;state.lastEliminatedId=target.id;state.history.push({round:state.round,name:target.name,id:target.id,time:Date.now()});await saveState();broadcast();return json(res,200,{targetId:target.id,targetName:target.name,targetChance:chances()[target.id],weights:Object.fromEntries(active.map(t=>[t.id,t.weight])),chances:chances(),state:publicState()})}
