...
function weightedPick(){ const {a,total}=activeWeighted(); if(!a.length||total<=0)return null; let r=Math.random()*total; for(const t of a){r-=t.amount;if(r<0)return t} return a[a.length-1]; }
function chances(){ const {a,total}=activeWeighted(); return Object.fromEntries(a.map(t=>[t.id,total?(t.amount/total*100):0])); }
